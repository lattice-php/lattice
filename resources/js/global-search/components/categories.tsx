      ...
      {categories.map((category) => (
        ...
        >
          <span className="flex items-center gap-2">
            {category.icon ? <Icon name={category.icon} aria-hidden="true" className="size-lt-icon-sm" /> : null}
            <span className="truncate">{category.label}</span>
          </span>
          {category.count !== null ? <span className="text-xs tabular-nums text-lt-muted-fg">{category.count}</span> : null}
        </button>
      ))}
    </div>
  );
