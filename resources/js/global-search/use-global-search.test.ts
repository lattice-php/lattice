...
const page1 = {
  data: [{ category: { name: "products" }, item: { id: "1", title: "Widget 1", subtitle: null, additionalInfo: null, link: "/products/1", badge: null } }],
...
const page2 = {
  data: [{ category: { name: "products" }, item: { id: "2", title: "Widget 2", subtitle: null, additionalInfo: null, link: "/products/2", badge: null } }],
...
describe("useGlobalSearch", () => {
  ...
  it("debounces the query then fetches results", async () => {
    ...
    const { result } = renderHook(() => useGlobalSearch({ endpoint: "/lattice/global-search", perPage: 1 }));
    act(() => result.current.setQuery("wid"));

    expect(fetchMock).not.toHaveBeenCalled(); // still within debounce window
    await act(async () => { await vi.advanceTimersByTimeAsync(GLOBAL_SEARCH_DEBOUNCE_MS); });
  ...
  it("records the selection over POST then navigates to the re-resolved link", async () => {
    ...
    const recordResponse = {
      data: { category: { name: "products" }, item: { id: "9", title: "Canonical", subtitle: null, additionalInfo: null, link: "/products/9", badge: null } },
    ...
    const clicked = { category: { name: "products" }, item: { id: "9", title: "Client Title", subtitle: null, additionalInfo: null, link: "/client/9", badge: null } };
