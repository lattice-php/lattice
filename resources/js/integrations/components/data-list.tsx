...
export function RemoteDataList({ props }: { props: DataListProps }) {
  ...
  useEffect(() => {
    ...
    void remoteJson<ExternalPayload>(props.dataEndpoint, { remote: props.remote })
      .then((payload) => {
        if (!cancelled) {
  ...
  return (
    <div className="rounded-lt border border-lt-border bg-lt-bg">
      <div className="border-b border-lt-border px-3 py-2 text-sm font-medium">External data</div>
...
export const DataList: RendererComponent<"remote.data-list"> = ({ node }) => {
  return <RemoteDataList props={node.props as DataListProps} />;
};
