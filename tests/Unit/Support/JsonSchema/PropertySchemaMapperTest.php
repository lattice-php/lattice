...
it('resolves a foreign def to a cross-document $ref when currentOrigin differs', function (): void {
    $context = new JsonSchemaContext(
        defNames: [Color::class => 'Color'],
        defOrigins: ['Color' => 'core'],
        schemaIds: ['core' => 'https://lattice-php.dev/schema/core/v1.json', 'table' => 'https://lattice-php.dev/schema/table/v1.json'],
        currentOrigin: 'table',
    );

    expect(new PropertySchemaMapper($context)->map(new ReflectionProperty(BadgeColumn::class, 'colors')))->toBe([
        'anyOf' => [
            [
                'type' => 'object',
                'additionalProperties' => ['$ref' => 'https://lattice-php.dev/schema/core/v1.json#/$defs/Color'],
                'x-lattice' => ['keys' => 'integer|string'],
            ],
            ['type' => 'null'],
        ],
...
it('keeps a same-document def local even when defOrigins/currentOrigin are set', function (): void {
    $context = new JsonSchemaContext(
        defNames: [Color::class => 'Color'],
        defOrigins: ['Color' => 'core'],
        schemaIds: ['core' => 'https://lattice-php.dev/schema/core/v1.json'],
        currentOrigin: 'core',
    );
