...
it('owns the core backend in an independently installable package', function (): void {
    ...
    expect($package['name'])->toBe('lattice-php/core')
        ...
        ->and(new ReflectionClass(LatticeRegistry::class)->getFileName())->toStartWith($packageRoot.'/src/')
        ->and(new ReflectionClass(Lattice::class)->getFileName())->toStartWith($packageRoot.'/src/');
});
