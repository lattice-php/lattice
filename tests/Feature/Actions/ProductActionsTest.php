...
test('the product archive row action authorizes per row', function (): void {
    ...
    // The render-time gate already hides this action for an archived row (see
    // Authorization/RenderAuthorizationTest.php), so a legitimate render never
    // produces a sealed ref here. Seal one directly to prove the endpoint still
    // enforces its own authorize() check regardless — defense in depth against a
    // forged or stale ref.
