...
it('scaffolds a column class, a cell tsx and registers it in columns.ts', function () {
    ...
    $php = File::get(app_path('Tables/Columns/StatusBadge.php'));
    expect($php)
        ->toContain('namespace App\\Tables\\Columns;')
        ->toContain('use Lattice\\Lattice\\Attributes\\Component;')
        ->toContain("#[Component('column.status-badge')]")
...
it('is idempotent and honors --type', function () {
    ...
    expect(File::get(app_path('Tables/Columns/Priority.php')))->toContain("#[Component('column.prio')]");
});
