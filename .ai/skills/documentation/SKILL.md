...
`docs:build` runs `npm run analyze` first (Sonda bundle-bench builds that write one
`docs/public/bundle-report-<target>.html` per analyze target — the framework plus each first-party
package), so the first run is slower and leaves those generated reports behind — expected, not a
change to commit.
