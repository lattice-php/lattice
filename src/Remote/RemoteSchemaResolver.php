...
final readonly class RemoteSchemaResolver
{
    ...
    public function resolve(RemoteSourceDefinition $definition, RemoteSchemaEndpoint $endpoint, Request $request): array
    {
        ...
        return $this->normalizer->normalize(
            $this->trustedManifest($manifest, $source, $endpoint),
            ['source' => $source],
        );
    }

    private function load(RemoteSchemaEndpoint $endpoint, Request $request): mixed
    {
        if ($endpoint->isFile()) {
    ...
    private function guardAllowedHost(RemoteSchemaEndpoint $endpoint): void
    {
        $host = parse_url($endpoint->uri, PHP_URL_HOST);

        if (! is_string($host) || $host === '') {
            throw new InvalidRemoteSchema("Remote schema endpoint [{$endpoint->uri}] must include a host.");
        }

        if ($endpoint->allowedHosts !== [] && ! in_array($host, $endpoint->allowedHosts, true)) {
            throw new InvalidRemoteSchema("Remote schema endpoint host [{$host}] is not allowed.");
        }
    }
    ...
    {
        if (! is_array($payload)) {
            throw new InvalidRemoteSchema('Remote schema payload must be a JSON object or array.');
        }

        $schema = $payload['schema'] ?? $payload;

        if (! is_array($schema) || ! array_is_list($schema)) {
            throw new InvalidRemoteSchema('Remote schema payload must contain a schema list.');
        }

        foreach ($schema as $node) {
            if (! is_array($node)) {
                throw new InvalidRemoteSchema('Remote schema payload must contain only object nodes.');
            }
        }
     ...
     * @return list<array<string, mixed>>
     */
    private function trustedManifest(array $manifest, string $source, RemoteSchemaEndpoint $endpoint): array
    {
        return array_map(
            fn (array $node): array => $this->trustedNode($node, $source, $endpoint),
            $manifest,
        );
    }
     ...
     */
    private function trustedNode(array $node, string $source, RemoteSchemaEndpoint $endpoint): array
    {
        $type = $node['type'] ?? null;
        $props = $node['props'] ?? [];
        ...
        if (is_array($props)) {
            ...
            if (is_string($type) && array_key_exists($type, self::EXTERNAL_URL_PROPS)) {
                ...
                $props['remote'] = $this->remoteAccess($node, $props, $source, $type);
        ...
        if (is_array($schema) && array_is_list($schema)) {
            $node['schema'] = array_map(
                fn (mixed $child): mixed => is_array($child) ? $this->trustedNode($child, $source, $endpoint) : $child,
                $schema,
            );
        }
     ...
     */
    private function remoteAccess(array $node, array $props, string $source, string $type): RemoteAccess
    {
        $nodeId = $this->nodeId($node, $type);
        $audience = $props['audience'] ?? null;

        if (! is_string($audience) || trim($audience) === '') {
            throw new InvalidRemoteSchema("Remote schema node [{$type}] must include an audience.");
        }
    ...
    private function nodeId(array $node, string $type): string
    {
        ...
        if (! is_string($nodeId) || $nodeId === '') {
            throw new InvalidRemoteSchema("Remote schema node [{$type}] must include an id or key.");
        }
    ...
    private function scopes(mixed $scopes): array
    {
        if (! is_array($scopes) || ! array_is_list($scopes)) {
            throw new InvalidRemoteSchema('Remote schema scopes must be a list of strings.');
        }
    ...
    private function guardAllowedRemoteUrl(RemoteSchemaEndpoint $endpoint, string $url, string $property): void
    {
        ...
        if ($endpoint->allowedHosts !== [] && ! in_array($host, $endpoint->allowedHosts, true)) {
            throw new InvalidRemoteSchema("Remote schema {$property} host [{$host}] is not allowed.");
        }
    }
}
