...
final class RemoteSchemaNormalizer
{
    ...
    private function node(array $node, string $path, array $context): Component
    {
        ...
        if (! is_string($type) || $type === '') {
            throw new InvalidArgumentException("Remote schema node at [{$path}] is missing a string type.");
        }

        $props = $node['props'] ?? [];
        if (! is_array($props)) {
            throw new InvalidArgumentException("Remote schema node at [{$path}] has invalid props.");
        }

        $children = $node['schema'] ?? [];
        if (! is_array($children)) {
            throw new InvalidArgumentException("Remote schema node at [{$path}] has invalid schema.");
        }

        $normalized = array_map(
            fn (mixed $child, int|string $index): Component => is_array($child)
                ? $this->node($child, "{$path}.schema.{$index}", $context)
                : throw new InvalidArgumentException("Remote schema node at [{$path}.schema.{$index}] is invalid."),
            $children,
            array_keys($children),
        );

        $key = is_string($node['key'] ?? null) ? $node['key'] : null;
        $component = new RemoteNode(
            key: $key,
            nodeType: $type,
            props: $props,
            schema: $normalized,
