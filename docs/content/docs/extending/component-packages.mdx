...
- **`css`** — the plugin aliases it as `@<vendor>/<name>/css` (`lattice-php/signature-example` → `@lattice-php/signature-example/css`); the consumer imports that specifier from their Tailwind entry. See [Styling](#styling).
...
The consumer adds an `@import` for the aliased specifier to their Tailwind entry, after the Lattice
css import:

```css
@import "@lattice-php/lattice/css";
@import "@acme/signature/css";
```

The `@source` line makes the consumer's Tailwind build scan the package's components, so arbitrary
