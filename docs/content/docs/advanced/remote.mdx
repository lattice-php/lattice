...
<Warning title="The manifest is untrusted input">
  ...
  server-trusted `remote` descriptor with a freshly sealed `ref`, so a remote service can never forge
  a usable token endpoint. External URL props (a data list's `dataEndpoint`, a chat box's
