...
provider directly. The provider stays in control of what it hands out, and the embedding app never
proxies the data.
...
| Role | Responsibility |
| --- | --- |
| **Consumer** | Renders the page and embeds the remote component. Hosts the token endpoint that mints browser tokens for its logged-in user. |
| **Provider** | Serves the data endpoint (and optionally the schema manifest) that the browser calls with the issued token. |
...
<Mermaid
  ...
  code={`sequenceDiagram
    autonumber
    participant P as Consumer page (PHP)
    participant B as Browser (React)
    participant T as Consumer token endpoint
    participant R as Remote provider
    P->>P: Build DataList / resolve schema manifest
    Note over P: seal ref, resolve tokenEndpoint
    P->>B: props.remote { source, audience, scopes, tokenEndpoint, ref }
    B->>T: POST tokenEndpoint — ref header, session cookie
    T->>T: verify ref, match source/audience/scopes, authorize
    T-->>B: BrowserToken { accessToken, tokenType, expiresIn }
    B->>R: GET dataEndpoint — Authorization Bearer, credentials omitted
    R-->>B: { data: [ ... ] }
    B->>B: render rows via dataBindings`}
/>

The consumer's server seals a tamper-proof `ref` that binds the node identity to a `source`,
`audience`, and `scopes`. The browser sends that `ref` (as a header, with the session cookie) to the
consumer's own token endpoint, which re-checks every bound value before asking the matching source to
issue a token. The provider call carries only the resulting bearer token — `credentials` are omitted,
so no cookies leak across the origin boundary.
...
and the provider's data endpoint:
...
Each row in the provider's `{ "data": [...] }` payload is materialized through the child schema.
...
[`chat.box`](/core/components/) component extends the remote machinery, so you opt in by calling
...
A **source** describes a provider. Extend `RemoteSourceDefinition`, tag it with `#[AsRemoteSource]`,
...
```json
{
  ...
  "schema": [
    { "type": "card", "props": { "dataBindings": { "title": "title" } } }
  ]
}
```

<Warning title="The manifest is untrusted input">
  `RemoteSchemaResolver` treats everything the provider returns as hostile. It strips any
  `ref`, `remote`, `endpoint`, `tokenEndpoint`, and `action` keys from the manifest and re-stamps a
  server-trusted `remote` descriptor with a freshly sealed `ref`, so a provider can never forge a
  usable token endpoint. External URL props (a data list's `dataEndpoint`, a chat box's
  ...
  `Accept-Language` is forwarded when fetching the manifest so the provider can localize it.
</Warning>
