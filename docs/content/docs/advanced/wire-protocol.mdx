...
model; it is never committed or versioned itself. Two projections come out of it in the same build:
...
"No drift" holds because both projections are produced from the same model in the same run — a
shape that isn't in the schema doesn't exist on the wire, and a shape the TypeScript module exports
is always backed by a schema def.
