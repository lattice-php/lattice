...
`->searchable()` turns the select into a server-driven search. The closure receives the query string
(plus the current `FormData` and `Request`) and returns the matching options. The option value is the
entity identifier and is fully controlled by the resolver, so it scales to large datasets without
shipping every option to the client.
