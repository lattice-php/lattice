...
The typed label becomes the value — free-text tags submit as a plain array of strings, and the
consumer creates matching records in `handle()`.
...
mixes real ids (picked options) with new labels (typed tags) — the consumer resolves the array in
`handle()`, keeping the ids and creating records for the remaining labels.
