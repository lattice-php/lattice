...
`Chart::make($title)` renders a data chart. Feed it rows with `->data()`, name the category axis with
`->categoryKey()`, then declare one or more series — `->line()`, `->bar()`, `->area()`, or `->pie()`
(see [`ChartSeriesType`](/advanced/enums/#charts)). Each series takes the data key it plots, an optional
display name, and an optional color; stack bars or areas by sharing a `stackId`.
...
Toggle the chrome with `->legend()`, `->tooltip()`, `->grid()`, `->xAxis()`, and `->yAxis()`. For a
dynamic set of series, pass `ChartSeries` value objects to `->series([...])` instead of the per-type
helpers.
