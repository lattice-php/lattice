...
<Info>
  Failed action requests dispatch a `lattice:action-error` DOM event carrying the error. Listen for it
  to add global error handling or observability — a toast, a Sentry report — without touching each
  action.
</Info>
