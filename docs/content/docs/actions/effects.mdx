...
<Info>
  ...
  `flash.toast` bag). Any code using that trait should switch to
  `Effects::flash(Effects::toast(...))`.
</Info>

## How effects reach the client

The result serializes to `{ ok, data, effects }`. Each effect carries a `type` discriminant
(`toast`, `callout`, `reload-component`, `reload-page`, `redirect`, `download`, `open-modal`,
`close-modal`, `reset-form`, `locale-change`, `toggle-sidebar`) and its payload; the client dispatches
them in order.
...
{
    public function __construct(public int $pieces = 100) {}
}
```
