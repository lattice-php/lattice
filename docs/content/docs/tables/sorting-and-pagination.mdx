...
<Info>
  `sort` and `page`/`per_page` are among the params a [URL-synced
  table](/tables/filtering/#syncing-filters-to-the-url) restores from — and writes back to — the
  page URL, so a sorted, paged view survives a refresh.
</Info>
