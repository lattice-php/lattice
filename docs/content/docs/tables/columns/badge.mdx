...
A cell whose value is a list — a JSON array cast, say — renders one pill per item, each labelled
through the column's [`options()`](/tables/columns/overview/#value-labels) and coloured through its
`colors()` map.
