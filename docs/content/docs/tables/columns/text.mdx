---
title: Text column
description: Render a value as text, with date/time formatting, links, copy-to-clipboard, and badge rendering.
---

import ComponentExample from "@components/ComponentExample.astro";
import Info from "@components/Info.astro";

`TextColumn` renders a value as text. It is sortable and filterable, and carries display modifiers for
the common value types a text cell holds.

<ComponentExample
  php={`TextColumn::make('name')->label('Name')->link('/people/{value}')->sortable();
TextColumn::make('email')->label('Email')->link('mailto:{value}')->copyable();
TextColumn::make('joined_at')->label('Joined')->date()->sortable();`}
  fixture="table.text"
/>

## Dates and times

`->date()`, `->time()`, and `->dateTime()` format a date/time value for the viewer's locale and
timezone. Pass a [`DateTimeStyle`](/advanced/enums/#tables) case — `Short`, `Medium` (the default),
`Long`, or `Full` — to control the style. A dated column filters as a [date](/tables/filtering/).

```php
use Lattice\Ui\Enums\DateTimeStyle;

TextColumn::make('created_at')->date();
TextColumn::make('updated_at')->dateTime(DateTimeStyle::Short);
```

## Links

`->link('/products/{value}')` renders the value as a link; `{value}` is substituted with the cell
value, and any other `{key}` in braces with that row field. Pass `external: true` for an outbound
link that opens in a new tab.

```php
TextColumn::make('email')->link('mailto:{value}')->copyable();
TextColumn::make('homepage')->link('{value}', external: true);
```

When a template is not enough, pass a closure instead — it runs on the server per row, receives
`$row` (the projected row array) and `$value` (this column's value) by name, and returns the href.
Return `null` to leave that row's cell as plain text.

```php
TextColumn::make('name')->link(fn (array $row): string => route('customers.show', $row['id']));
```

## Popover

`->popover()` renders the value as a trigger that opens a [popover](/components/popover/). The
closure runs on the server per row — receiving `$row` and `$value` like a link closure — and
returns the popover's content component, or `null` for no popover on that row. Combined with a
lazy [fragment](/core/fragments/), the content loads from the server when the popover first opens:

```php
TextColumn::make('customer_name')->popover(fn (array $row) => Fragment::lazy(
    CustomerCardFragment::class,
    ['customerId' => $row['customer_id']],
));
```

<Info>A column carries either a link or a popover, never both.</Info>

## Copyable

`->copyable()` adds a click-to-copy affordance to the cell.

```php
TextColumn::make('api_token')->copyable();
```

## As a badge

`->badge()` renders the value as a coloured pill. The colour is read from a sibling field on the row —
`->badge('status_color')` reads it from `status_color`, defaulting to the `color` field. That field
may hold a colour name (`green`, `success`, …) or any CSS colour — a hex value stored by a
[ColorPicker](/forms/fields/color-picker/) field works as-is — falling back to gray when unset or
unrecognized. For a fixed value-to-colour map instead, use a dedicated
[Badge column](/tables/columns/badge/).

```php
TextColumn::make('status')->badge('status_color');
```

## To-many relations

`->multiple('name')` draws the value from a to-many relation named by the column key, reading `name`
from each related row and rendering the values as a list. It filters through the relation and is never
sortable — see [relation columns](/tables/eloquent-tables/#relation-columns).

```php
TextColumn::make('tags')->multiple('name')->badge('color');
```
