...
For a notification that also needs other channels — mail, Slack, SMS — extend `LatticeNotification`
instead of building one inline. Implement `toLattice()` once and it backs both the database row and
the broadcast payload; override `via()` to add channels:
...
class OrderShipped extends LatticeNotification
{
...
`LatticeNotification` defaults `via()` to `['database', 'broadcast']` — call
`parent::via($notifiable)` and append to it rather than replacing the list, or the bell stops
