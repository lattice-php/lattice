...
<ComponentExample
  php={`Chart::make('Revenue')
    ...
    ->line('revenue', 'Revenue')
    ->categoryFormat(DateFormat::date(DateTimeStyle::Short))   // raw dates → short, localized
...
`NumberFormat::currency($code)`, and `->unit(NumberFormatUnit::Percent)`. `DateFormat::date()`,
`::time()`, and `::dateTime()` take a `DateTimeStyle` (Full/Long/Medium/Short) and format with the
active locale and timezone — so you pass **raw dates** instead of pre-translated labels.
