---
title: Charts
description: Data charts — line, bar, area, pie, doughnut, and gauge series that serialize to Recharts.
---
...
<Warning>
  Pies and gauges can't share a chart with cartesian series. If a chart declares any line, bar, or
  area series, only those render and the pie or gauge is dropped. Declaring several pie or gauge
  series together renders only the first. Give each its own chart.
</Warning>
...
<ComponentExample
  php={`Chart::make('Revenue by channel', 'channel-mix-doughnut-chart')
    ->description('Share of total revenue')
    ->data([
        ['channel' => 'Direct', 'amount' => 42000, 'color' => '#2563eb'],
        ['channel' => 'Partner', 'amount' => 27000, 'color' => '#16a34a'],
...
<ComponentExample
  php={`Chart::make('CPU usage', 'cpu-gauge-chart')
...
`var(--lt-primary)` to stay theme-aware. For pies and gauges, a per-row `color` field wins over the
series color.
